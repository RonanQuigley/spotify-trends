...
const SpotifyApi = function(){
  ...
  function getTopArtists(accessToken, limit, offset, onRequestCompleted) {
    const storedResultsObj = {
      fourWeeks: null,
      sixMonths: null,
      allTime: null
    };
    const callback = function(timeRange, result){
    ...
    getRequest(accessToken, limit, offset, callback, requestType.ARTISTS);
  }
  
  function getTopTracks(accessToken, limit, offset, onRequestCompleted) {
    const storedResultsObj = {
      fourWeeks: null,
      sixMonths: null,
      allTime: null
    };
    const callback = (timeRange, result) => {
      storeResults(result, storedResultsObj, timeRange);
      if (storedResultsObj.allTime && storedResultsObj.sixMonths && storedResultsObj.fourWeeks) {
        onRequestCompleted(storedResultsObj);
      }
    };
    getRequest(accessToken, limit, offset, callback, requestType.TRACKS);
  }
  ...
  function getRequest(accessToken, limit, offset, callback, requestedDataType) {
    ...
    const createRequest = function(i) {
