export default {
    artists: {
        SHORT: {
            items: [
                {
                    external_urls: {
                        spotify:
                            'https://open.spotify.com/artist/00oL7zWxmWveTsKF7DnIRd'
                    },
                    followers: { href: null, total: 27761 },
                    genres: ['soundtrack'],
                    href:
                        'https://api.spotify.com/v1/artists/00oL7zWxmWveTsKF7DnIRd',
                    id: '00oL7zWxmWveTsKF7DnIRd',
                    images: [
                        {
                            height: 640,
                            url:
                                'https://i.scdn.co/image/802b7c0501bc87aac70106fe0a2de9dd6598744d',
                            width: 640
                        },
                        {
                            height: 300,
                            url:
                                'https://i.scdn.co/image/984e723d433769e33d35070793c02e62440721d4',
                            width: 300
                        },
                        {
                            height: 64,
                            url:
                                'https://i.scdn.co/image/167d936f19ea4215d5b1f6f7a78f43608d92a962',
                            width: 64
                        }
                    ],
                    name: 'Kyle Dixon & Michael Stein',
                    popularity: 60,
                    type: 'artist',
                    uri: 'spotify:artist:00oL7zWxmWveTsKF7DnIRd'
                }
            ],
            total: 14,
            limit: 1,
            offset: 0,
            href:
                'https://api.spotify.com/v1/me/top/artists?limit=1&offset=0&time_range=short_term',
            previous: null,
            next:
                'https://api.spotify.com/v1/me/top/artists?limit=1&offset=1&time_range=short_term'
        },
        MEDIUM: {
            items: [
                {
                    external_urls: {
                        spotify:
                            'https://open.spotify.com/artist/6XyY86QOPPrYVGvF9ch6wz'
                    },
                    followers: { href: null, total: 8348334 },
                    genres: [
                        'alternative metal',
                        'nu metal',
                        'post-grunge',
                        'rap metal'
                    ],
                    href:
                        'https://api.spotify.com/v1/artists/6XyY86QOPPrYVGvF9ch6wz',
                    id: '6XyY86QOPPrYVGvF9ch6wz',
                    images: [
                        {
                            height: 640,
                            url:
                                'https://i.scdn.co/image/1685533969d5b68cbc630f991e873bd6467f1814',
                            width: 640
                        },
                        {
                            height: 320,
                            url:
                                'https://i.scdn.co/image/f759994946aa42851e5293083f472c96c1753105',
                            width: 320
                        },
                        {
                            height: 160,
                            url:
                                'https://i.scdn.co/image/0b0925b544b46d90a549f25a7f754ce6e59e6be2',
                            width: 160
                        }
                    ],
                    name: 'Linkin Park',
                    popularity: 85,
                    type: 'artist',
                    uri: 'spotify:artist:6XyY86QOPPrYVGvF9ch6wz'
                }
            ],
            total: 50,
            limit: 1,
            offset: 0,
            href: 'https://api.spotify.com/v1/me/top/artists?limit=1&offset=0',
            previous: null,
            next: 'https://api.spotify.com/v1/me/top/artists?limit=1&offset=1'
        },
        LONG: {
            items: [
                {
                    external_urls: {
                        spotify:
                            'https://open.spotify.com/artist/6XyY86QOPPrYVGvF9ch6wz'
                    },
                    followers: { href: null, total: 8348334 },
                    genres: [
                        'alternative metal',
                        'nu metal',
                        'post-grunge',
                        'rap metal'
                    ],
                    href:
                        'https://api.spotify.com/v1/artists/6XyY86QOPPrYVGvF9ch6wz',
                    id: '6XyY86QOPPrYVGvF9ch6wz',
                    images: [
                        {
                            height: 640,
                            url:
                                'https://i.scdn.co/image/1685533969d5b68cbc630f991e873bd6467f1814',
                            width: 640
                        },
                        {
                            height: 320,
                            url:
                                'https://i.scdn.co/image/f759994946aa42851e5293083f472c96c1753105',
                            width: 320
                        },
                        {
                            height: 160,
                            url:
                                'https://i.scdn.co/image/0b0925b544b46d90a549f25a7f754ce6e59e6be2',
                            width: 160
                        }
                    ],
                    name: 'Linkin Park',
                    popularity: 85,
                    type: 'artist',
                    uri: 'spotify:artist:6XyY86QOPPrYVGvF9ch6wz'
                }
            ],
            total: 50,
            limit: 1,
            offset: 0,
            previous: null,
            href:
                'https://api.spotify.com/v1/me/top/artists?limit=1&offset=0&time_range=long_term',
            next:
                'https://api.spotify.com/v1/me/top/artists?limit=1&offset=1&time_range=long_term'
        }
    },
    tracks: {
        SHORT: {
            items: [
                {
                    album: {
                        album_type: 'ALBUM',
                        artists: [
                            {
                                external_urls: {
                                    spotify:
                                        'https://open.spotify.com/artist/7MSUfLeTdDEoZiJPDSBXgi'
                                },
                                href:
                                    'https://api.spotify.com/v1/artists/7MSUfLeTdDEoZiJPDSBXgi',
                                id: '7MSUfLeTdDEoZiJPDSBXgi',
                                name: 'Brian Eno',
                                type: 'artist',
                                uri: 'spotify:artist:7MSUfLeTdDEoZiJPDSBXgi'
                            }
                        ],
                        external_urls: {
                            spotify:
                                'https://open.spotify.com/album/1Z2jkEtW5Sc9wWVxUgyG0E'
                        },
                        href:
                            'https://api.spotify.com/v1/albums/1Z2jkEtW5Sc9wWVxUgyG0E',
                        id: '1Z2jkEtW5Sc9wWVxUgyG0E',
                        images: [
                            {
                                height: 640,
                                url:
                                    'https://i.scdn.co/image/881a150b6e7e35d5c3162cc17cbceb647a217636',
                                width: 640
                            },
                            {
                                height: 300,
                                url:
                                    'https://i.scdn.co/image/122477fb15a5cea16a602481a54aea2f6c67ff5f',
                                width: 300
                            },
                            {
                                height: 64,
                                url:
                                    'https://i.scdn.co/image/15824dbb40131d8c081373fb1b2613a16c584f0d',
                                width: 64
                            }
                        ],
                        name: 'Apollo',
                        release_date: '1983-07-01',
                        release_date_precision: 'day',
                        type: 'album',
                        uri: 'spotify:album:1Z2jkEtW5Sc9wWVxUgyG0E'
                    },
                    artists: [
                        {
                            external_urls: {
                                spotify:
                                    'https://open.spotify.com/artist/7MSUfLeTdDEoZiJPDSBXgi'
                            },
                            href:
                                'https://api.spotify.com/v1/artists/7MSUfLeTdDEoZiJPDSBXgi',
                            id: '7MSUfLeTdDEoZiJPDSBXgi',
                            name: 'Brian Eno',
                            type: 'artist',
                            uri: 'spotify:artist:7MSUfLeTdDEoZiJPDSBXgi'
                        }
                    ],
                    disc_number: 1,
                    duration_ms: 266200,
                    explicit: false,
                    external_ids: { isrc: 'GBAAA0500103' },
                    external_urls: {
                        spotify:
                            'https://open.spotify.com/track/1vgSaC0BPlL6LEm4Xsx59J'
                    },
                    href:
                        'https://api.spotify.com/v1/tracks/1vgSaC0BPlL6LEm4Xsx59J',
                    id: '1vgSaC0BPlL6LEm4Xsx59J',
                    is_local: false,
                    is_playable: true,
                    name: 'An Ending (Ascent) - 2005 Digital Remaster',
                    popularity: 67,
                    preview_url: null,
                    track_number: 5,
                    type: 'track',
                    uri: 'spotify:track:1vgSaC0BPlL6LEm4Xsx59J'
                }
            ],
            total: 50,
            limit: 1,
            offset: 0,
            previous: null,
            href:
                'https://api.spotify.com/v1/me/top/tracks?limit=1&offset=0&time_range=short_term',
            next:
                'https://api.spotify.com/v1/me/top/tracks?limit=1&offset=1&time_range=short_term'
        },
        MEDIUM: {
            items: [
                {
                    album: {
                        album_type: 'ALBUM',
                        artists: [
                            {
                                external_urls: {
                                    spotify:
                                        'https://open.spotify.com/artist/0YC192cP3KPCRWx8zr8MfZ'
                                },
                                href:
                                    'https://api.spotify.com/v1/artists/0YC192cP3KPCRWx8zr8MfZ',
                                id: '0YC192cP3KPCRWx8zr8MfZ',
                                name: 'Hans Zimmer',
                                type: 'artist',
                                uri: 'spotify:artist:0YC192cP3KPCRWx8zr8MfZ'
                            },
                            {
                                external_urls: {
                                    spotify:
                                        'https://open.spotify.com/artist/2xOp0rCDPAmYqnL2UFbaDY'
                                },
                                href:
                                    'https://api.spotify.com/v1/artists/2xOp0rCDPAmYqnL2UFbaDY',
                                id: '2xOp0rCDPAmYqnL2UFbaDY',
                                name: 'Benjamin Wallfisch',
                                type: 'artist',
                                uri: 'spotify:artist:2xOp0rCDPAmYqnL2UFbaDY'
                            }
                        ],
                        external_urls: {
                            spotify:
                                'https://open.spotify.com/album/5kSUsy5FU3Wcxd4DBvXFm4'
                        },
                        href:
                            'https://api.spotify.com/v1/albums/5kSUsy5FU3Wcxd4DBvXFm4',
                        id: '5kSUsy5FU3Wcxd4DBvXFm4',
                        images: [
                            {
                                height: 640,
                                url:
                                    'https://i.scdn.co/image/71ba954049737d56477cc150c348e223ebfcfa03',
                                width: 640
                            },
                            {
                                height: 300,
                                url:
                                    'https://i.scdn.co/image/1722c88ab0b6d77b54a15de8323da51684ec39e9',
                                width: 300
                            },
                            {
                                height: 64,
                                url:
                                    'https://i.scdn.co/image/dcb324c52ef91fb584e84efa18a5c694809af4de',
                                width: 64
                            }
                        ],
                        name:
                            'Blade Runner 2049 (Original Motion Picture Soundtrack)',
                        release_date: '2017-10-05',
                        release_date_precision: 'day',
                        type: 'album',
                        uri: 'spotify:album:5kSUsy5FU3Wcxd4DBvXFm4'
                    },
                    artists: [
                        {
                            external_urls: {
                                spotify:
                                    'https://open.spotify.com/artist/0YC192cP3KPCRWx8zr8MfZ'
                            },
                            href:
                                'https://api.spotify.com/v1/artists/0YC192cP3KPCRWx8zr8MfZ',
                            id: '0YC192cP3KPCRWx8zr8MfZ',
                            name: 'Hans Zimmer',
                            type: 'artist',
                            uri: 'spotify:artist:0YC192cP3KPCRWx8zr8MfZ'
                        },
                        {
                            external_urls: {
                                spotify:
                                    'https://open.spotify.com/artist/2xOp0rCDPAmYqnL2UFbaDY'
                            },
                            href:
                                'https://api.spotify.com/v1/artists/2xOp0rCDPAmYqnL2UFbaDY',
                            id: '2xOp0rCDPAmYqnL2UFbaDY',
                            name: 'Benjamin Wallfisch',
                            type: 'artist',
                            uri: 'spotify:artist:2xOp0rCDPAmYqnL2UFbaDY'
                        }
                    ],
                    disc_number: 1,
                    duration_ms: 231918,
                    explicit: false,
                    external_ids: { isrc: 'QMCXZ1700073' },
                    external_urls: {
                        spotify:
                            'https://open.spotify.com/track/33MhyD4tOUTPDrJtUOmlzy'
                    },
                    href:
                        'https://api.spotify.com/v1/tracks/33MhyD4tOUTPDrJtUOmlzy',
                    id: '33MhyD4tOUTPDrJtUOmlzy',
                    is_local: false,
                    is_playable: true,
                    name: 'Joi',
                    popularity: 47,
                    preview_url:
                        'https://p.scdn.co/mp3-preview/23e3bd73ade17288a18768806902dfa8ad6e2e15?cid=null',
                    track_number: 12,
                    type: 'track',
                    uri: 'spotify:track:33MhyD4tOUTPDrJtUOmlzy'
                }
            ],
            total: 50,
            limit: 1,
            offset: 0,
            href: 'https://api.spotify.com/v1/me/top/tracks?limit=1&offset=0',
            previous: null,
            next: 'https://api.spotify.com/v1/me/top/tracks?limit=1&offset=1'
        },
        LONG: {
            items: [
                {
                    album: {
                        album_type: 'ALBUM',
                        artists: [
                            {
                                external_urls: {
                                    spotify:
                                        'https://open.spotify.com/artist/6XyY86QOPPrYVGvF9ch6wz'
                                },
                                href:
                                    'https://api.spotify.com/v1/artists/6XyY86QOPPrYVGvF9ch6wz',
                                id: '6XyY86QOPPrYVGvF9ch6wz',
                                name: 'Linkin Park',
                                type: 'artist',
                                uri: 'spotify:artist:6XyY86QOPPrYVGvF9ch6wz'
                            }
                        ],
                        external_urls: {
                            spotify:
                                'https://open.spotify.com/album/2tlTBLz2w52rpGCLBGyGw6'
                        },
                        href:
                            'https://api.spotify.com/v1/albums/2tlTBLz2w52rpGCLBGyGw6',
                        id: '2tlTBLz2w52rpGCLBGyGw6',
                        images: [
                            {
                                height: 640,
                                url:
                                    'https://i.scdn.co/image/deb48b2781de449175ff7865da3bb249652fc28f',
                                width: 640
                            },
                            {
                                height: 300,
                                url:
                                    'https://i.scdn.co/image/25c88542f89cfcd842e62b6c3851518fe0eca6f6',
                                width: 300
                            },
                            {
                                height: 64,
                                url:
                                    'https://i.scdn.co/image/98ab8557be40ee514e38af4f6f1f7af8f5c447fc',
                                width: 64
                            }
                        ],
                        name: 'Minutes To Midnight (Explicit Version)',
                        release_date: '2007-05-14',
                        release_date_precision: 'day',
                        type: 'album',
                        uri: 'spotify:album:2tlTBLz2w52rpGCLBGyGw6'
                    },
                    artists: [
                        {
                            external_urls: {
                                spotify:
                                    'https://open.spotify.com/artist/6XyY86QOPPrYVGvF9ch6wz'
                            },
                            href:
                                'https://api.spotify.com/v1/artists/6XyY86QOPPrYVGvF9ch6wz',
                            id: '6XyY86QOPPrYVGvF9ch6wz',
                            name: 'Linkin Park',
                            type: 'artist',
                            uri: 'spotify:artist:6XyY86QOPPrYVGvF9ch6wz'
                        }
                    ],
                    disc_number: 1,
                    duration_ms: 209306,
                    explicit: false,
                    external_ids: { isrc: 'USWB10701212' },
                    external_urls: {
                        spotify:
                            'https://open.spotify.com/track/0sp00HSXkQyqTa6QqM0O8V'
                    },
                    href:
                        'https://api.spotify.com/v1/tracks/0sp00HSXkQyqTa6QqM0O8V',
                    id: '0sp00HSXkQyqTa6QqM0O8V',
                    is_local: false,
                    is_playable: true,
                    name: 'Leave Out All The Rest',
                    popularity: 68,
                    preview_url:
                        'https://p.scdn.co/mp3-preview/ebe5e431132a55286d6cd339a0d73cb8f0873ff4?cid=null',
                    track_number: 3,
                    type: 'track',
                    uri: 'spotify:track:0sp00HSXkQyqTa6QqM0O8V'
                }
            ],
            total: 50,
            limit: 1,
            offset: 0,
            previous: null,
            href:
                'https://api.spotify.com/v1/me/top/tracks?limit=1&offset=0&time_range=long_term',
            next:
                'https://api.spotify.com/v1/me/top/tracks?limit=1&offset=1&time_range=long_term'
        }
    }
};
